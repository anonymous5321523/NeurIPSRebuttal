...
def main(args):
    ...
    dataset = pd.read_pickle('prompts_url.pickle')
    
    prompts = [p for p in dataset.keys()]
    ...
    progress_bar = tqdm(range(len(prompts)), desc="Processing", unit="batch")

    for i in progress_bar:
        seed = i + args.gen_seed

        prompt = prompts[i].replace('_',' ')
        ...
        urls = dataset[prompts[i]]
        gt_images = [Image.open(BytesIO(requests.get(url, verify = False, headers=headers).content)).convert('RGB') for url in urls]
        ...
        metadata = {'SSCD': sim, 'CLIP': clip_score, 'prompt': prompt, 'Aesthetic Score': aesthetic_score}
